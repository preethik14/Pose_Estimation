...
class ImageLoader(object):
    def __init__(self, filepath: str):
        ...
        if filepath.startswith('camera'):
            ...
            self.mode = 'camera'
        elif filepath.startswith('rtsp://'):  # Check for RTSP links
            self.cap = cv2.VideoCapture(filepath)  # Open the RTSP stream
            if not self.cap.isOpened():
                raise IOError(f"Can't open RTSP stream {filepath}!")
            logging.info(f'Opened RTSP stream {filepath}')
            self.mode = 'rtsp'
    ...
    def __getitem__(self, item):
        if self.mode == 'camera' or self.mode == 'video':
            ...
            if not ret or img is None:
                raise Exception("Can't read image from RTSP stream")   
        return img
